...
favoriteRouter.route('/')
    ...
    .get(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        Favorites.find({})
            ...
            .then((favorites) => {
                if (favorites) {
                    user_fav = favorites.filter((fav) => fav.user._id.toString() === req.user.id.toString())[0]
                    if (!user_fav) {
                    ...
                    var err = new Error('There are no favorites on DB!')
                    err.statusCode = 404
                    return next(err)
                }
            }, (err) => next(err))
            ...
            .then((favorites) => {
                ...
                for (let i of req.body) {
                    if (user.dishes.find((d_id) => {
                        if (d_id._id) { return d_id._id.toString() === i._id.toString(); }
                    }))
                        continue;
    ...
    .delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        Favorites.find({})
            ...
            .then((favorites) => {
                ...
                if (user) {
                    user.dishes = user.dishes.filter((fav) => fav.user._id.toString() !== req.params.dishId)
                    user.save()
                        .then((result) => {
                            res.statusCode = 200
                            res.setHeader('Content-Type', 'application/json')
